				       ...
				       uint32_t comp_status)
{
	...
	if (ctx->sa.enc_type != AL_CRYPT_AES_CTR) {
		...
		/* IV OUT must be updated for AES-CTR */

		if (rctx->cryptlen >= ivsize) {
			unsigned int keylen;
			u8 pt[AES_BLOCK_SIZE];

			switch (ctx->sa.aes_ksize) {
			...
			print_hex_dump_debug("aes-ctr pt: ", DUMP_PREFIX_OFFSET, 16, 1, pt, ivsize, false);
			print_hex_dump_debug("aes-ctr ct: ", DUMP_PREFIX_OFFSET, 16, 1, req->iv, ivsize, false);
	...
	print_hex_dump_debug("out iv: ", DUMP_PREFIX_OFFSET, 16, 1, req->iv, ivsize, false);
